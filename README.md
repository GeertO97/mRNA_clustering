...
## Interactive dashboard for final images

An interactive Dash application is available to browse the final clustering images
generated by the notebooks or scripts in this repository. The dashboard scans a
directory for PNG, JPEG, or TIFF files and provides controls to filter files and
adjust the on-screen preview size.
...
2. Export the figures you want to review into a directory (for example
   `final_outputs/`).

3. Launch the dashboard, pointing it at the directory that contains the images:

   ```bash
   python -m dashboard.app --image-dir final_outputs
   ```

   The application will be available at http://127.0.0.1:8050/ by default. Use
   `--host`, `--port`, or `--title` to customise the server settings.
